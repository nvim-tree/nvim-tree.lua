...
function Renderer:draw()
  local bufnr = self.explorer.window:get_bufnr()
  if not bufnr or not vim.api.nvim_buf_is_loaded(bufnr) then
    return
  end
  ...
  local cursor = vim.api.nvim_win_get_cursor(self.explorer.window:get_winnr() or 0)
  ...
  if cursor and #builder.lines >= cursor[1] then
    vim.api.nvim_win_set_cursor(self.explorer.window:get_winnr() or 0, cursor)
  end

  self.explorer.window:grow_from_content()

  log.profile_end(profile)

  events._dispatch_on_tree_rendered(bufnr, self.explorer.window:get_winnr())
end
